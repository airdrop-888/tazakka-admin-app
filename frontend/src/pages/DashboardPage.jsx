// frontend/src/pages/DashboardPage.jsx (KODE LENGKAP - PERBAIKAN FINAL)
...
function DashboardPage({ token, onLogout }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError('');
    try {
        const apiUrl = "/api/reports/daily/";
  ...
  const handleApiSubmit = async (endpoint, data, successCallback) => {
    try {
      ...
      // --- PERBAIKAN 1: URL HARUS DINAMIS BERDASARKAN ENDPOINT YANG DIKIRIM ---
      const apiUrl = `/api${endpoint}`; // Contoh: menjadi /api/transactions/
  ...
  const handleDelete = async (type, id) => {
    if (window.confirm("Anda yakin ingin menghapus data ini? Aksi ini tidak dapat dibatalkan.")) {
        try {
            // --- PERBAIKAN 2: URL HARUS MENUNJUK KE ITEM SPESIFIK YANG AKAN DIHAPUS ---
            const apiUrl = `/api/${type}/${id}`; // Contoh: menjadi /api/transactions/123
  ...
  const handleExportXLSX = async () => {
      ...
      try {
        // --- PERBAIKAN 3: HAPUS VITE_API_BASE_URL DAN GUNAKAN PATH RELATIF ---
        const apiUrl = `/api/reports/monthly/export?year=${year}&month=${month}`;
